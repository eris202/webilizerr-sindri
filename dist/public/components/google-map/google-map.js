...
function SimpleGoogleMap(data) {
    ({ node, markers } = data, params = __rest(data, ["node", "markers"]));
    ...
    if (typeof (this.params.center) === 'string') {
        this.map = new google.maps.Map(this.node, Object.assign(Object.assign({}, this.params), { center: { lat: 0, lng: 0 } }));
...
SimpleGoogleMap.prototype.setMarker = function (id) {
    let marker = this.markers[id];
    marker.instance = new google.maps.Marker(Object.assign({ map: this.map }, marker));
    if (marker.html) {
...
SimpleGoogleMap.prototype.filter = function (tags) {
    let ids = Object.getOwnPropertyNames(this.markers), filtered = [];
    if (tags.includes('*')) {
        filtered = ids;
    }
    else {
        filtered = ids.filter((function (id) {
            ...
            for (let i = 0; i < marker.tags.length; i++)
                if (tags.includes(marker.tags[i]))
                    return true;
            return false;
        }).bind(this));
    }
