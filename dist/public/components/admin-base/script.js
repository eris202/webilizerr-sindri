...
let components = {
    ...
    theme: {
        ...
        init: function () {
            window.theme = new Theme({
                name: 'greendesk-navbar',
                localStorage: true,
                values: {
                    'navbar-sidebar-color': '#fff',
                    'navbar-sidebar-bg': '#58a193',
                    'navbar-toggle-color': '#58a193',
    ...
    themeSwitcher: {
        ...
        dependencies: 'theme',
        init: function (nodes) {
            let switchStorage = window.localStorage.getItem('greendesk-theme-switch');
            nodes.forEach(function (node) {
                let switchName = node.getAttribute('data-theme-switch'), switchTheme = JSON.parse(node.getAttribute('data-theme'));
                if (switchStorage) {
                    ...
                    if (node.classList.contains('active')) {
                        window.activeSwitch = node;
                        window.localStorage.setItem('greendesk-theme-switch', switchName);
                    }
                }
    ...
    sessionTimeout: {
        ...
        init: function () {
            let loginPage = 'index.html', timer = document.querySelector('[data-session-timer]'), btnLogout = document.querySelector('[data-session-logout]'), btnStay = document.querySelector('[data-session-stay]'), sessionModal = $('#session-modal'), countdown = new Counter({
                timeout: 15,
                checkpoint: 10,
    ...
    adminPanel: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                ...
                new AdminPanel(Object.assign(Object.assign({ themes: ['default', 'primary', 'secondary', 'success', 'info', 'warning', 'danger', 'light', 'dark'] }, parseJSON(node.getAttribute('data-admin-panel'))), { node: node }));
                if (panelSwitch) {
                    MultiSwitch({
                        node: panelSwitch,
                        targets: [node]
    ...
    fontRoboto: {
        selector: 'html',
        styles: 'https://fonts.googleapis.com/css?family=Roboto:400,500,700&display=swap'
    },
    fontAwesome: {
        selector: '[class*="fa-"]',
    ...
    spinner: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                $(node).spinner(Object.assign({ min: 0, step: 1 }, parseJSON(node.getAttribute('data-spinner'))));
            });
        }
    },
    timeSpinner: {
    ...
    daterangepiker: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                let $node = $(node), params = parseJSON(node.getAttribute('data-params'));
                $node.daterangepicker(Object.assign(Object.assign({ opens: 'left', cancelButtonClasses: 'btn-outline-light', applyButtonClasses: 'btn-warning' }, params), { ranges: node.hasAttribute('data-predefined') ? {
                        'Today': [moment(), moment()],
    ...
    datetimepicker: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                let $node = $(node), params = parseJSON($node.attr('data-datetimepicker')), defaults = {
                    format: 'L LT',
                ...
                if ((device.ios() || device.android()) && !params.inline) {
                    ...
                    }).bind($node), inputClickHandler = (function (event) {
                        event.preventDefault();
                ...
                $node.datetimepicker(Object.assign(Object.assign({}, defaults), params));
            });
        }
    },
    tagManager: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                $(node.querySelector('.tag-manager-input')).tagsManager(Object.assign(Object.assign({ prefilled: ["Smartphone", "Apple", "Notebook", "Gadget"], tagClass: 'badge badge-primary' }, parseJSON(node.getAttribute('data-tag-manager'))), { tagsContainer: node.querySelector('.tag-manager-container') }));
            });
        }
    },
    tooltip: {
    ...
    classSwitcher: {
        ...
        init: function (nodes) {
            ...
            nodes.forEach(function (node) {
                let group, params = parseJSON(node.getAttribute('data-class-switcher')), container = document.querySelector(params.group), target = container.querySelector(params.target);
                ...
                if (params.targetClass)
                    group.targetClass = params.targetClass;
                if (params.triggerClass)
                    group.triggerClass = params.triggerClass;
                if (node.classList.contains(group.triggerClass))
                    group.lastTrigger = node;
                node.classSwitcher = {
                    group: group,
                    target: target
                };
                node.addEventListener('click', function () {
                    let group = this.classSwitcher.group, event = new CustomEvent('classSwitching', { bubbles: true });
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                ...
                node.addEventListener('click', function () {
                    ...
                    if (modal.animClass)
                        modal.classList.remove(modal.animClass);
                    modal.animClass = params.animClass;
                    if (modal.animClass)
                        modal.classList.add(modal.animClass);
    ...
    colorpicker: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                $(node).colorpicker(Object.assign({ fallbackColor: '#000' }, parseJSON(node.getAttribute('data-colorpick'))));
            });
        }
    },
    maskedinput: {
    ...
    summernote: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                $(node).summernote(Object.assign({ airMode: false, placeholder: 'Write here...', blockquoteBreakingLevel: 1, focus: false }, parseJSON(node.getAttribute('data-summernote-opts'))));
            });
        }
    },
    markdown: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                $(node).markdown(Object.assign({ iconlibrary: 'fa', height: '350', resize: 'vertical' }, parseJSON(node.getAttribute('data-markdown-params'))));
            });
        }
    },
    nestable: {
    ...
    jstree: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                let params = Object.assign({ types: {
                        '#': {
                            'valid_children': ['root']
                        },
                        'root': {
    ...
    cropper: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                let image = node.querySelector('.img-container img'), options = {
                    aspectRatio: 16 / 9,
                    preview: node.querySelectorAll('.img-preview'),
                }, cropper = new Cropper(image, options);
                node.querySelectorAll('[data-cropper]').forEach(function (node) {
                    node.addEventListener('click', function () {
                        let params = parseJSON(this.getAttribute('data-cropper')), target = null, cropBoxData = cropper.getCropBoxData(), canvasData = cropper.getCanvasData();
                        ...
                        for (let key in params) {
                            ...
                            if (['crop', 'reset', 'clear', 'replace', 'enable', 'disable', 'destroy', 'move', 'moveTo', 'zoom', 'zoomTo', 'rotate', 'rotateTo', 'scale', 'scaleX', 'scaleY', 'setAspectRatio', 'setDragMode'].indexOf(key) !== -1) {
                                if (params[key] instanceof Array)
                                    cropper[key](params[key][0], params[key][1]);
                                else
                                    cropper[key](params[key]);
                            }
                            // Options handling
    ...
    fileupload: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                $(node).fileInput(Object.assign({ iconClass: 'mdi-upload' }, parseJSON(node.getAttribute('data-tfi-opts'))));
            });
        }
    },
    easyzoom: {
    ...
    slick: {
        ...
        init: function (nodes) {
            ...
            let addButton = document.querySelector('.add-slide'), removeButton = document.querySelector('.remove-slide'), filterButton = document.querySelector('.filter-slide');
            // Add sample
            if (addButton)
            ...
            if (switches.length)
                switches.forEach(function (trigger) {
    ...
    dropzone: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                new Dropzone(node, Object.assign({ paramName: "file", addRemoveLinks: true, dictRemoveFile: '<span class="fa-close"></span>', autoQueue: true }, parseJSON(node.getAttribute('data-dropzone'))));
            });
        }
    },
    fullscreen: {
    ...
    animationSample: {
        ...
        init: function (nodes) {
            let timeoutId = null, endCb = null, target = document.querySelector('#animation-sample');
            nodes.forEach(function (node) {
                node.addEventListener('click', function () {
                    if (timeoutId)
                        clearTimeout(timeoutId);
    ...
    pnotify: {
        ...
        init: function () {
            ...
            window.pnotifyStacks = {
                stackTopLeft: Object.assign(Object.assign({}, PNotify.defaultStack), { dir1: 'down', dir2: 'right' }),
                stackBottomLeft: Object.assign(Object.assign({}, PNotify.defaultStack), { dir1: 'up', dir2: 'right' }),
                stackBottomRight: Object.assign(Object.assign({}, PNotify.defaultStack), { dir1: 'up', dir2: 'left' }),
                stackCenterTop: Object.assign(Object.assign({}, PNotify.defaultStack), { dir1: 'down', dir2: null }),
                stackCenterBottom: Object.assign(Object.assign({}, PNotify.defaultStack), { dir1: 'up', dir2: null })
            };
            PNotify.styling.bootstrap4 = Object.assign(Object.assign({}, PNotify.styling.bootstrap4), { primary: 'alert-primary', secondary: 'alert-secondary', warning: 'alert-warning', danger: 'alert-danger', dark: 'alert-dark', light: 'alert-light' });
            PNotify.icons.fontawesome4 = Object.assign(Object.assign({}, PNotify.icons.fontawesome4), { primary: 'fa fa-exclamation-triangle', secondary: 'fa fa-exclamation-triangle', warning: 'fa fa-exclamation-triangle', danger: 'fa fa-exclamation-triangle', dark: 'fa fa-exclamation-triangle', light: 'fa fa-exclamation-triangle' });
        }
    },
    pnotifyButton: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                let params = Object.assign(Object.assign({ type: 'primary', title: 'Notification Sample', text: 'Fidelis, primus resistentias unus imperium de rusticus.', animation: 'fade', width: '300px' }, parseJSON(node.getAttribute('data-pnotify'))), { styling: 'bootstrap4', icons: 'fontawesome4' });
                if (params.stack) {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                ...
                switch (node.getAttribute('data-table-mode')) {
                    case 'print':
                        opts = Object.assign(Object.assign({}, opts), { dom: `<'row row-10'<'col-sm-12 col-md-6 pl-3'B><'col-sm-12 col-md-6 pr-3'f>>
							<'row'<'col-sm-12'tr>>
							<'row row-10'<'col-sm-12 col-md-5 pl-3'i><'col-sm-12 col-md-7 pr-3'p>>`, buttons: [
                                {
                                    extend: 'print',
                                    className: 'btn-dark',
                    ...
                    case 'multi-filter':
                        ...
                        opts = Object.assign(Object.assign({}, opts), { ordering: false, searching: true });
                        // Apply the search
                        ...
                        break;
                    case 'row-grouping':
                        var groupColumn = 2, table1 = $(node).DataTable(Object.assign(Object.assign({}, opts), { "columnDefs": [
                                { "visible": false, "targets": groupColumn }
                            ], "order": [[groupColumn, 'asc']], "displayLength": 25, "drawCallback": function (settings) {
                                var api = this.api();
                    ...
                    case 'col-reorder':
                        ...
                        break;
                    case 'collapse':
                        var table2 = $(node).DataTable(Object.assign(Object.assign({}, opts), { "ajax": "components/datatables/objects.txt", "columns": [
                                {
    ...
    driver: {
        ...
        init: function (nodes) {
            ...
            }, start = document.querySelector('[data-driver-start]'), steps = [], driver;
            nodes.forEach(function (node) {
                steps.push({
                    element: node,
    ...
    timeline: {
        ...
        init: function (nodes) {
            function resizeHandler() {
                let items = this.querySelectorAll('.timeline-item'), tmp = [null, null];
                items.forEach(function (item, i) {
                    let top = item.offsetTop, offset = Math.ceil(item.getBoundingClientRect().height + top);
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                ...
                $(node).fullCalendar({
                    ...
                    header: Object.assign({ left: 'prev,next today', center: 'title', right: 'month,agendaWeek,agendaDay' }, parseJSON(node.getAttribute('data-fullcalendar-header'))),
                    editable: true,
                    droppable: true,
                    drop: function () {
    ...
    jvectorMap: {
        ...
        init: function (nodes) {
            nodes.forEach(function (container) {
                let map = new jvm.Map(Object.assign({ container: $(container).find('.jvector-map-body') }, parseJSON(container.getAttribute('data-jvector-map'))));
                // flag select toggles
    ...
    flotchart: {
        ...
        init: function (nodes) {
            let options = {
                colors: ['#58a193', '#f7ad6a', '#5b5eab', '#34b1cc', '#48c8af', '#ff5f68'],
                grid: {
                    show: true,
                    aboveData: true,
                    color: '#bebebe',
    ...
    gmap: {
        ...
        init: function (nodes) {
            let promises = [];
            nodes.forEach(function (node) {
                let sMap = new SimpleGoogleMap(Object.assign({ node: node, center: { lat: 0, lng: 0 }, zoom: 4 }, parseJSON(node.getAttribute('data-settings'))));
    ...
    gmapMarkerInfo: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                node.addEventListener('click', function () {
                    let params = parseJSON(this.getAttribute('data-marker-info')), map = document.querySelector(params.mapId).simpleGoogleMap;
    ...
    gmapFilter: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                node.addEventListener('click', function () {
                    let params = parseJSON(this.getAttribute('data-map-filter')), map = document.querySelector(params.mapId).simpleGoogleMap;
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                node.addEventListener('submit', function (event) {
                    event.preventDefault();
                    let input = this.querySelector('input'), map = document.querySelector(this.getAttribute('data-map-search')).simpleGoogleMap;
    ...
    gmapPanorama: {
        ...
        init: function (nodes) {
            let promises = [];
            nodes.forEach(function (node) {
                let sMap = document.querySelector(node.getAttribute('data-map-panorama')).simpleGoogleMap, panorama = new google.maps.StreetViewPanorama(node, {
    ...
    gmapNavigation: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                let map = document.querySelector(node.getAttribute('data-map-navigation')).simpleGoogleMap, prev = node.querySelector('[data-map-prev]'), next = node.querySelector('[data-map-next]'), title = node.querySelector('[data-map-title]'), currentId = '', setNavigation = function (id) {
                ...
                prev.addEventListener('click', function () {
                    let list = Object.getOwnPropertyNames(map.markers), current = list.indexOf(currentId), next = current <= 0 ? list.length - 1 : current - 1;
                    setNavigation(list[next]);
                });
                next.addEventListener('click', function () {
                    let list = Object.getOwnPropertyNames(map.markers), current = list.indexOf(currentId), next = current >= list.length - 1 ? 0 : current + 1;
                    setNavigation(list[next]);
                });
            });
        }
    ...
    multiswitch: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                MultiSwitch(Object.assign({ node: node, event: device.ios() ? 'touchstart' : 'click' }, parseJSON(node.getAttribute('data-multi-switch'))));
            });
        }
    },
    navigationSwitch: {
        ...
        init: function (nodes) {
            nodes.forEach(function (node) {
                let timeoutId, mSwitch, resizeTrigger = function () {
                    clearTimeout(timeoutId);
                    timeoutId = setTimeout(function () {
                        window.dispatchEvent(new Event('resize'));
                    }, 300);
...
function initComponent(component) {
    let stylesState = Promise.resolve(), scriptsState = Promise.resolve();
    //{DEL DIST}
...
function initComponents(components) {
    let stylesPromises = [], scriptsPromises = [];
